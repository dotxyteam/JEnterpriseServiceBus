...
/*
 *  Implements a simple highlight painter that renders squiggles under the
...
public class SquigglePainter extends DefaultHighlighter.DefaultHighlightPainter {
	...
	public Shape paintLayer(Graphics g, int offs0, int offs1, Shape bounds, JTextComponent c, View view) {
		...
		// if drawing area width is too small but not null, then extend it a little
		if (r.width > 0) {
			if (r.width < twoSquiggles) {
				r.width = twoSquiggles;
			}
		}
