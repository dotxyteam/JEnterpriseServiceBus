...
public class Schedule extends Activator{
	...
	private Variant<DateTime> startDateTimeVariant=new Variant<DateTime>(DateTime.class, DateTime.fromJavaUtilDate(new Date(System.currentTimeMillis() + 60000)));
	...
	public Variant<DateTime> getStartDateTimeVariant() {
		return startDateTimeVariant;
	}
	
	public void setStartDateTimeVariant(Variant<DateTime> startDateTimeVariant) {
	...
	public void initializeAutomaticTrigger(ActivationHandler activationHandler) throws Exception {
		...
		Runnable task = new Runnable() {
			@Override
			public void run() {
				activationHandler.trigger(new InputClassStructure(DateTime.now()));
			}
		};
	...
	public static void customizeUI(InfoCustomizations infoCustomizations) {
		// Schedule form customization
		{
			// field control positions
			InfoCustomizations.getTypeCustomization(infoCustomizations, Schedule.class.getName())
			.setCustomFieldsOrder(Arrays.asList("startDateTimeVariant", "repeatingVariant", "repetitionSettings", "timeZoneIdentifierVariant"));
			// startDateTimeVariant control customization
			{
				InfoCustomizations.getFieldCustomization(infoCustomizations, Schedule.class.getName(), "startDateTimeVariant")
				.setCustomFieldCaption("Start At");
				InfoCustomizations.getTypeCustomization(InfoCustomizations.getFieldCustomization(infoCustomizations, com.otk.jesb.ui.GUI.VariantCustomizations.getAdapterTypeName(Schedule.class.getName(),"startDateTimeVariant"), com.otk.jesb.ui.GUI.VariantCustomizations.getConstantValueFieldName("startDateTimeVariant"))
				.getSpecificTypeCustomizations(), DateTime.class.getName()).setSpecificProperties(new java.util.HashMap<String, Object>() {
					private static final long serialVersionUID = 1L;
					{
						ReflectionUIUtils.setFieldControlPluginIdentifier(this, "com.otk.jesb.ui.GUI$JESDDateTimePickerPlugin");
						ReflectionUIUtils.setFieldControlPluginConfiguration(this, "com.otk.jesb.ui.GUI$JESDDateTimePickerPlugin", (Serializable) com.otk.jesb.PluginBuilder.readControlPluginConfiguration("<xy.reflect.ui.control.swing.plugin.DateTimePickerPlugin_-DateTimePickerConfiguration><dateFormat>yyyy-MM-dd</dateFormat><timeFormat>HH:mm:ss</timeFormat></xy.reflect.ui.control.swing.plugin.DateTimePickerPlugin_-DateTimePickerConfiguration>"));
					}
				});
			}
			// repeatingVariant control customization
			{
				InfoCustomizations.getFieldCustomization(infoCustomizations, Schedule.class.getName(), "repeatingVariant")
				.setCustomFieldCaption("Repeat");
			}
			// repetitionSettings control customization
			{
				InfoCustomizations.getFieldCustomization(infoCustomizations, Schedule.class.getName(), "repetitionSettings")
				.setCustomFieldCaption("Repetition Settings");
				InfoCustomizations.getFieldCustomization(infoCustomizations, Schedule.class.getName(), "repetitionSettings")
				.setValueValidityDetectionForced(true);
				InfoCustomizations.getFieldCustomization(infoCustomizations, Schedule.class.getName(), "repetitionSettings").setFormControlEmbeddingForced(true);
				RepetitionSettingsStructure.customizeUI(infoCustomizations);
			}
			// timeZoneIdentifierVariant control customization
			{
				InfoCustomizations.getFieldCustomization(infoCustomizations, Schedule.class.getName(), "timeZoneIdentifierVariant")
				.setCustomFieldCaption("Time Zone");
			}
			// hide UI customization method
			InfoCustomizations.getMethodCustomization(infoCustomizations, Schedule.class.getName(), ReflectionUIUtils.buildMethodSignature("void", "customizeUI", Arrays.asList(InfoCustomizations.class.getName())))
			.setHidden(true);
		}
	}
		...
		if(repeatingVariant.getValue()){
		    ...
		    if(repetitionSettings.endDateTimeVariant.getValue() != null){
		        if(!repetitionSettings.endDateTimeVariant.getValue().toJavaUtilDate().after(startDateTimeVariant.getValue().toJavaUtilDate())){
		            throw new ValidationError("Invalid schedule end date/time: It is not after the schedule start date/time");
		        }
		        if(!repetitionSettings.endDateTimeVariant.getValue().toJavaUtilDate().after(new Date())){
		            throw new ValidationError("Invalid schedule end date/time: It has passed");
		        }
		    }
		}else{
		    if(!startDateTimeVariant.getValue().toJavaUtilDate().after(new Date())){
	...
	private ScheduledFuture<?> prepare(Runnable task, Date start, boolean repeating, long periodLength,
			ChronoUnit periodUnit, Date end) {
		Date now = new Date();
		Date next;
		if (repeating) {
	...
	private Date computeNextOccurrence(Date start, long periodLength, ChronoUnit periodUnit,
			Date now) {
		if (start.after(now)) {
			return start;
		}
		...
		return Date.from(startZonedDateTime.plus(missed * periodLength, periodUnit).toInstant());
	}
	
	
	static public class InputClassStructure{
	
		public final DateTime activationMoment;
		
		public InputClassStructure(DateTime activationMoment){
	...
	static public class RepetitionSettingsStructure implements ActivatorStructure{
		...
		private Variant<RepetitionSettingsStructurePeriodUnitStructure> periodUnitVariant=new Variant<RepetitionSettingsStructurePeriodUnitStructure>(RepetitionSettingsStructurePeriodUnitStructure.class, RepetitionSettingsStructurePeriodUnitStructure.DAYS);
		private Variant<Long> periodLengthVariant=new Variant<Long>(Long.class, 1l);
		private Variant<DateTime> endDateTimeVariant=new Variant<DateTime>(DateTime.class, null);
		...
		public Variant<DateTime> getEndDateTimeVariant() {
			return endDateTimeVariant;
		}
		
		public void setEndDateTimeVariant(Variant<DateTime> endDateTimeVariant) {
			this.endDateTimeVariant = endDateTimeVariant;
		}
		
		@Override
		public String toString() {
			return "RepetitionSettingsStructure [periodUnitVariant=" + periodUnitVariant + ", periodLengthVariant=" + periodLengthVariant + ", endDateTimeVariant=" + endDateTimeVariant + "]";
		}
		...
		public static void customizeUI(InfoCustomizations infoCustomizations) {
			// RepetitionSettingsStructure form customization
			{
				...
				.setCustomFieldsOrder(Arrays.asList("periodUnitVariant", "periodLengthVariant", "endDateTimeVariant"));
				...
				{
					InfoCustomizations.getFieldCustomization(infoCustomizations, RepetitionSettingsStructure.class.getName(), "endDateTimeVariant")
					.setCustomFieldCaption("End At");
					InfoCustomizations.getFieldCustomization(infoCustomizations, com.otk.jesb.ui.GUI.VariantCustomizations.getAdapterTypeName(RepetitionSettingsStructure.class.getName(),"endDateTimeVariant"), com.otk.jesb.ui.GUI.VariantCustomizations.getConstantValueFieldName("endDateTimeVariant"))
					.setNullValueDistinctForced(true);
					InfoCustomizations.getTypeCustomization(InfoCustomizations.getFieldCustomization(infoCustomizations, com.otk.jesb.ui.GUI.VariantCustomizations.getAdapterTypeName(RepetitionSettingsStructure.class.getName(),"endDateTimeVariant"), com.otk.jesb.ui.GUI.VariantCustomizations.getConstantValueFieldName("endDateTimeVariant"))
					.getSpecificTypeCustomizations(), DateTime.class.getName()).setSpecificProperties(new java.util.HashMap<String, Object>() {
						private static final long serialVersionUID = 1L;
						{
							ReflectionUIUtils.setFieldControlPluginIdentifier(this, "com.otk.jesb.ui.GUI$JESDDateTimePickerPlugin");
				...
				// hide UI customization method
				InfoCustomizations.getMethodCustomization(infoCustomizations, RepetitionSettingsStructure.class.getName(), ReflectionUIUtils.buildMethodSignature("void", "customizeUI", Arrays.asList(InfoCustomizations.class.getName())))
				.setHidden(true);
			}
		}
